...
then
	...
	#If not then run the same program again but with "-c" option as the affect will be excatly same.
		...
		if [ $v_present = 1 ]
		then
			bash -f main.sh -cvf $target_file $@
		else
			bash -f main.sh -cf $target_file $@
		fi
		exit
	fi
	...
	declare -a new_file_list=($(bash -f main.sh -tf $target_file | tr " " " "))
